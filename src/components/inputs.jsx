function Inputs({ inputFields, sectionData, handleInputChange, currentSection }) {
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
      {inputFields.map((field, index) => (
        <div key={index} className="flex flex-col">
          {/* Label */}
          <label htmlFor={`${currentSection}-${field.name}`} className="text-lg font-medium text-gray-200 mb-2">
            {field.label}
          </label>
          ...
          <input
            ...
            onChange={(e) => handleInputChange(currentSection, field.name, e.target.value)}
